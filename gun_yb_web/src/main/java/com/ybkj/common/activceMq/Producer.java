...
public class Producer {
    ...
    */
     ...
     * @return
     */
    public BaseModel sendMessageAppSetUpParameters(String appImei,ParamSettingMessageBody body) throws Exception {
        BaseModel baseModel = new BaseModel();
        try {
            ...
            message.setDeviceType("2");//设备类型：1.离位置报警设备 2.随行设备 3.腕表 4.定位模块
            message.setSerialNumber(dateTool.dateToString() + progressiveIncreaseNumber.getRandomNumber());//交易流水号
            message.setMessageType("27");//报文类型
            message.setSendTime(dateTool.dateToString());//发报时间
            message.setSessionToken(TokenUtils.channelSessionDigest());//标识本次会话唯一标志


            messageBody.setBroadcastInterval(body.getBroadcastInterval());
            messageBody.setConnectionInterval(body.getConnectionInterval());
            messageBody.setConnectionTimeout(body.getConnectionTimeout());
            messageBody.setHeartbeat(body.getHeartbeat());
            messageBody.setMatchTime(body.getMatchTime());
            messageBody.setPositioningInterval(body.getPositioningInterval());
            messageBody.setPowerAlarmLevel(body.getPowerAlarmLevel());
            messageBody.setPowerSampling(body.getPowerSampling());
            messageBody.setSafeCode(body.getSafeCode());
            messageBody.setSoftwareDeviceVersion(body.getSoftwareDeviceVersion());
            messageBody.setTransmittingPower(body.getTransmittingPower());


            messageBody.setSystemTime(dateTool.dateToString());
            messageBody.setAuthCode(TokenUtils.channelSessionDigest());
            message.setMessageBody(messageBody);
            String jsonString = JSONObject.toJSONString(message, SerializerFeature.DisableCircularReferenceDetect);
            jmsMessagingTemplate.convertAndSend(storageQueue, jsonString);
